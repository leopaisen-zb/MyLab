...
# 移除 ocpmodels 依赖，添加 PyTorch Geometric 兼容性
try:
    from torch_geometric.nn.acts import swish          # PyG < 2.0
except ImportError:                                    # PyG ≥ 2.0
    from torch_geometric.nn.resolver import activation_resolver
    swish = activation_resolver("swish")

# 简单的距离编码实现，替代 ocpmodels.models.scn.smearing
class GaussianSmearing(nn.Module):
    def __init__(self, start=0.0, stop=5.0, num_gaussians=50, width=2.0):
        super().__init__()
        self.start = start
        self.stop = stop
...
# 简单的条件梯度装饰器，替代 ocpmodels.common.utils.conditional_grad
...
class EquiformerV2_OC20(nn.Module):
    ...
    def generate_graph(self, data):
        """使用 radius_graph 动态生成图结构"""
        pos = data.pos
        batch = getattr(data, 'batch', None)
        
        # 检查是否已有预计算的边
        if hasattr(data, 'edge_index') and hasattr(data, 'edge_distance'):
            edge_index = data.edge_index
            edge_distance_vec = pos[edge_index[1]] - pos[edge_index[0]]
            edge_distance = data.edge_distance
        else:
            # 使用 radius_graph 动态建图
            try:
                ...
                # 如果没有 torch_cluster，使用简单的距离计算
                if batch is None:
                    # 单个分子情况
                    num_atoms = pos.size(0)
                    edge_index = torch.combinations(torch.arange(num_atoms), 2).t()
                ...
                # 过滤距离
                mask = edge_distance <= self.max_radius
