...
with RAG-enhanced Text→VASP 数据集 (`text_rag`) 作为训练数据，并从已有 LoRA
checkpoint（例如 `checkpoint-3114`）继续训练。
"""
...
# 使用 text_rag 目录下的 RAG 增强数据集（直接 Text → VASP 结构）
TRAIN_JSON: Path = (
    PROJECT_ROOT
    / "data"
    / "rag_data"
    / "text_rag"
    / "dataset_train_rag.json"
)
...
def load_train_data(file_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(data, list):
            raise ValueError("训练数据根节点必须是 JSON 数组 (list)。")
        required = {"instruction", "input", "output"}
        for i, item in enumerate(data):
            if not isinstance(item, dict):
                raise ValueError(f"第 {i + 1} 条样本必须是对象 (dict)。")
            missing = required - set(item.keys())
            if missing:
                raise ValueError(
                    f"第 {i + 1} 条样本缺少字段: {missing}，需要 {required}。"
                )
...
def main() -> None:
    try:
        if not torch.cuda.is_available():
            raise RuntimeError(
                "CUDA 环境不可用，请检查显卡驱动或 PyTorch 安装状态。",
            )

        logger.info(
            "LoRA SFT - Qwen2.5-7B-Instruct with text_rag Code dataset "
            "(resume from latest checkpoint if available)",
        )

        # 使用本地模型路径时强制 HF 离线，避免因网络超时导致已缓存模型仍报错
        if os.path.isdir(_LOCAL_MODEL_PATH):
            os.environ["HF_HUB_OFFLINE"] = "1"
            logger.info(
                "Detected local model path; using HF_HUB_OFFLINE=1 (cache only).",
            )

        logger.info("Loading base model and tokenizer in BF16 precision...")
        ...
        logger.info("Formatting text_rag dataset into Unsloth compatible text field...")
        raw_data = load_train_data(TRAIN_JSON)
        if not raw_data:
            raise ValueError("训练数据为空，请检查数据文件或路径。")
        ...
        # 自动从 OUTPUT_DIR 下最新的 checkpoint 继续训练（仅匹配 checkpoint-<数字>）
        ...
        if OUTPUT_DIR.exists():
            ...
            logger.info(
                "Output directory does not exist. Starting training from scratch.",
            )
        ...
        logger.info("Model successfully saved to: %s", OUTPUT_DIR)
