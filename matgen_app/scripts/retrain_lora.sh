#!/usr/bin/env bash
# scripts/retrain_lora.sh
#
# 飞轮⑧：Qwen2.5-7B LoRA 生成模型一键重训脚本
#
# Demo 用法（答辩演示，无需 GPU，跳过真实训练直接注册 mock 版本）：
#   bash scripts/retrain_lora.sh --demo
#
# 真实训练（需要 GPU 环境 + 至少 24GB VRAM + Qwen2.5-7B 基础模型）：
